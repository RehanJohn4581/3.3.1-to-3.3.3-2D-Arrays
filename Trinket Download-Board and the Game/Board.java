...
public class Board
{  
  public static String[] tileValues = {"lion", "lion",
                                        "penguin", "penguin",
                                        "dolphin", "dolphin",
                                        "fox", "fox",
                                        "monkey", "monkey",
  ...
  public Board() {
        ...
        for (int r = 0; r < gameboard.length; r++) {
            for (int c = 0; c < gameboard[0].length; c++) {
                int rand = (int)(Math.random() * (tileCount + 1));
 ...
 public String toString()
{
  ...
  public void showValue (int row, int column)
  {
    ...
    /* your code here */
    gameboard[row][column].setShowing(true);
  }  
 ...
 public String checkForMatch(int row1, int col1, int row2, int col2)
{
    ...
    if (gameboard[row1][col1].getValue().equals(gameboard[row2][col2].getValue()))
    {
        ...
        gameboard[row1][col1].setMatched(true);
        gameboard[row2][col2].setMatched(true);
        gameboard[row1][col1].setShowing(true);
        gameboard[row2][col2].setShowing(true);

        msg = "Matched";
    }
    else
    {
        // Tiles do not match → hide them again
        gameboard[row1][col1].setShowing(false);
        gameboard[row2][col2].setShowing(false);
